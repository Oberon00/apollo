...
using to_type_of = typename detail::remove_cvr<Converter>::to_type;
...
struct lua_type_id<T,
        typename std::enable_if<
            std::is_same<T, detail::remove_cvr<T>>::value
            && (
...
template <typename T>
using push_converter_for = converter<detail::remove_cvr<T>>;
